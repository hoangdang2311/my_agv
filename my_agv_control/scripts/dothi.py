...
def animate(i):
    ...
    plt.suptitle("Oscillation graph of ...")
    ...
    plt.subplot(2, 1, 1)
    plt.plot(t, x_vals, linewidth=1, color='green', label="x_val")
    plt.plot(t, x_ref, linewidth=1, color='blue', label="x_ref")
    plt.ylabel('x')
    ...
    plt.subplot(2, 1, 2)
    plt.plot(t, z_vals, linewidth=1, color='green', label="z_val")
    plt.plot(t, z_ref, linewidth=1, color='blue', label="z_ref")
    plt.ylabel('z')
...
def subvel_callback(msg: Twist):
    ...
    Info.x1 = float(msg.linear.x)
    Info.z1 = float(msg.angular.z)

def velsub_callback(vel_data: Twist):
    rospy.loginfo("(" + str(vel_data.linear.x) + ", " + str(vel_data.angular.z) + ")")
    Info.x2 = float(vel_data.linear.x)
    Info.z2 = float(vel_data.angular.z)
